...
public class AdminService {
	...
	public ResponseEntity<AdminResponse> adminLogin(AdminLoginRequest adminLoginRequest){
		...
		Admin admin= adminRepository.findByLoginIdAndPassword(adminLoginRequest.getLoginId(), adminLoginRequest.getPassword());
		if(admin!=null) {
			return ResponseEntity.status(HttpStatus.OK).body(new AdminResponse("Login Successfully", true));
		}
		return ResponseEntity.status(HttpStatus.UNAUTHORIZED).body(new AdminResponse("Login Failed !! Invalid Id or Password", false));
	}
}
